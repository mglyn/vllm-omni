...
from vllm_omni.diffusion.models.ltx2.ops.diffvae import rope_3d as rope_3d_ops
...
@pytest.mark.skipif(not _sm90_available(), reason="CUDA SM90 required")
@pytest.mark.parametrize(
    ("batch", "frames", "height", "width", "heads"),
    [
        (2, 3, 7, 7, 1),
        (1, 31, 136, 192, 4),
    ],
)
def test_diffvae_qk_scale_rope_3d_is_bit_exact(
    batch: int,
    frames: int,
    height: int,
    width: int,
    ...
    shape = (batch, frames, height, width, heads, 64)
    query = torch.randn(shape, device="cuda", dtype=torch.bfloat16, generator=generator)
    key = torch.randn_like(query)
    tables = _tables(frames, height, width)
    expected = (
        _apply_rope_reference(query * 0.125, tables),
        _apply_rope_reference(key, tables),
    )

    with torch.inference_mode():
        actual = try_qk_scale_rope_3d_exact(query, key, 0.125, _DIM_SPLIT, 10000.0)
...
@pytest.fixture
def isolated_rope_runtime_state():
    failed_devices = set(rope_3d_ops._FAILED_DEVICES)
    verified_contracts = set(rope_3d_ops._VERIFIED_CONTRACTS)
    rope_3d_ops._FAILED_DEVICES.clear()
    rope_3d_ops._VERIFIED_CONTRACTS.clear()
    try:
        yield
    finally:
        rope_3d_ops._FAILED_DEVICES.clear()
        rope_3d_ops._FAILED_DEVICES.update(failed_devices)
        rope_3d_ops._VERIFIED_CONTRACTS.clear()
        rope_3d_ops._VERIFIED_CONTRACTS.update(verified_contracts)


@pytest.mark.skipif(not _sm90_available(), reason="CUDA SM90 required")
def test_diffvae_qk_scale_rope_3d_supports_nondefault_exact_contract(isolated_rope_runtime_state) -> None:
    dim_split = (8, 20, 36)
    scale = 0.375
    base = 8192.0
    shape = (1, 5, 9, 11, 2, 64)
    ...
    query = torch.randn(shape, device="cuda", dtype=torch.bfloat16, generator=generator)
    key = torch.randn_like(query)
    tables = _tables(shape[1], shape[2], shape[3], dim_split, base)
    expected = (
        _apply_rope_reference(query * scale, tables, dim_split),
        _apply_rope_reference(key, tables, dim_split),
    )

    with torch.inference_mode():
        actual = try_qk_scale_rope_3d_exact(query, key, scale, dim_split, base)
...
@pytest.mark.skipif(not _sm90_available(), reason="CUDA SM90 required")
def test_diffvae_qk_scale_rope_3d_verifies_each_math_contract_once(
    monkeypatch: pytest.MonkeyPatch,
    isolated_rope_runtime_state,
) -> None:
    query = torch.randn(1, 3, 7, 7, 1, 64, device="cuda", dtype=torch.bfloat16)
    key = torch.randn_like(query)
    reference_calls = 0
    reference = rope_3d_ops._reference
    ...
    monkeypatch.setattr(rope_3d_ops, "_reference", tracked_reference)
    contracts = (
        (_DIM_SPLIT, 0.125, 10000.0),
        (_DIM_SPLIT, 0.125, 10000.0),
        ((8, 20, 36), 0.375, 8192.0),
        ((8, 20, 36), 0.375, 8192.0),
    )
    with torch.inference_mode():
        for dim_split, scale, base in contracts:
            assert try_qk_scale_rope_3d_exact(query, key, scale, dim_split, base) is not None

    assert reference_calls == 2
    assert rope_3d_ops._VERIFIED_CONTRACTS == {
        (query.device.index, _DIM_SPLIT, 0.125, 10000.0),
        (query.device.index, (8, 20, 36), 0.375, 8192.0),
    }


def test_diffvae_qk_scale_rope_3d_rejects_unsupported_inputs() -> None:
    query = torch.randn(1, 3, 7, 7, 1, 64, dtype=torch.bfloat16)
    key = torch.randn_like(query)

    with torch.inference_mode():
        assert try_qk_scale_rope_3d_exact(query, key, 0.125, _DIM_SPLIT, 10000.0) is None


@pytest.mark.skipif(not _sm90_available(), reason="CUDA SM90 required")
def test_diffvae_qk_scale_rope_3d_falls_back_while_compiling(monkeypatch: pytest.MonkeyPatch) -> None:
    query = torch.randn(1, 3, 7, 7, 1, 64, device="cuda", dtype=torch.bfloat16)
    key = torch.randn_like(query)
    monkeypatch.setattr(torch.compiler, "is_compiling", lambda: True)
...
@pytest.mark.skipif(not _sm90_available(), reason="CUDA SM90 required")
def test_diffvae_qk_scale_rope_3d_permanently_falls_back_after_launch_failure(
    monkeypatch: pytest.MonkeyPatch,
    isolated_rope_runtime_state,
) -> None:
    query = torch.randn(1, 3, 7, 7, 1, 64, device="cuda", dtype=torch.bfloat16)
    ...
    monkeypatch.setattr(rope_3d_ops, "_launch", fail_launch)
    with torch.inference_mode():
        assert try_qk_scale_rope_3d_exact(query, key, 0.125, _DIM_SPLIT, 10000.0) is None
        assert try_qk_scale_rope_3d_exact(query, key, 0.125, _DIM_SPLIT, 10000.0) is None

    assert calls == 1
    assert query.device.index in rope_3d_ops._FAILED_DEVICES


@pytest.mark.skipif(not _sm90_available(), reason="CUDA SM90 required")
def test_diffvae_qk_scale_rope_3d_permanently_falls_back_after_mismatch(
    monkeypatch: pytest.MonkeyPatch,
    isolated_rope_runtime_state,
) -> None:
    query = torch.randn(1, 3, 7, 7, 1, 64, device="cuda", dtype=torch.bfloat16)
    ...
    def mismatched_launch(*args, **kwargs):
        nonlocal calls
        calls += 1
        return torch.zeros_like(query), torch.zeros_like(key)

    monkeypatch.setattr(rope_3d_ops, "_launch", mismatched_launch)
    with torch.inference_mode():
        assert try_qk_scale_rope_3d_exact(query, key, 0.125, _DIM_SPLIT, 10000.0) is None
        assert try_qk_scale_rope_3d_exact(query, key, 0.125, _DIM_SPLIT, 10000.0) is None
...
@pytest.mark.skipif(not _sm90_available(), reason="CUDA SM90 required")
def test_diffvae_qk_scale_rope_3d_falls_back_after_reference_failure(
    monkeypatch: pytest.MonkeyPatch,
    isolated_rope_runtime_state,
) -> None:
    query = torch.randn(1, 3, 7, 7, 1, 64, device="cuda", dtype=torch.bfloat16)
    ...
    monkeypatch.setattr(rope_3d_ops, "_launch", lambda *args, **kwargs: (query.clone(), key.clone()))

    def fail_reference(*args, **kwargs):
        raise RuntimeError("injected reference failure")

    monkeypatch.setattr(rope_3d_ops, "_reference", fail_reference)
    with torch.inference_mode():
        assert try_qk_scale_rope_3d_exact(query, key, 0.125, _DIM_SPLIT, 10000.0) is None
