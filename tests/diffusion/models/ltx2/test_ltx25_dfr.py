...
from vllm_omni.diffusion.models.ltx2.ltx2_denoise import LTXDenoiseContext, LTXPhaseResult
...
def test_dfr_final_decode_uses_ordinary_phase_result():
    assert "decode_keyframes" not in {field.name for field in fields(LTXPhaseResult)}
...
def test_dfr_profiles_each_phase_and_clears_conditioning():
    pipe = _tiny_dfr_pipe()
    phase = LTX25_DFR_RECIPE.phases[0]
    conditioning = _DFRVideoConditioning(generated_positions=(8,))
    profiled = []
    entered: list[object] = []
    expected = object()

    @contextmanager
    def record_profile(method_name, phase_name=None):
        profiled.append((phase_name, method_name))
        yield

    pipe._profile_phase_method = record_profile
    pipe._enter_phase = entered.append
    pipe.run_phase = lambda *args, **kwargs: expected

    actual = pipe._run_dfr_phase(
        None,
        _inputs(),
        phase,
        conditioning,
    ...
    assert profiled == [(phase.name, "run_phase")]
    assert pipe._dfr_conditioning is None
