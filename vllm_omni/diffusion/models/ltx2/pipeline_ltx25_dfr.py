...
This entry implements the ``spatial_upscalings=2`` flow only: a
...
this pipeline's request contract. Generated keyframes condition the denoising
stages; final video latents use the ordinary LTX-2.5 DiffVAE decode path.
"""
...
class LTX25DFRPipeline(LTXI2VConditioningMixin, LTXRuntime):
    """LTX-2.5 T/I2VA DFR path for 3840x2176 output at 24 fps."""
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        if not self.use_diffusion_decoder:
            raise ValueError("LTX25DFRPipeline requires the LTX-2.5 diffusion video decoder.")
    ...
    def _decode_lanczos_reencode_keyframes(
        ...
        for index in range(keyframes.shape[2]):
            ...
            with self._profile_phase_method("diffvae_decode", "keyframe_carry"):
    ...
    def _run_dfr_phase(
        ...
        *,
        ...
        try:
            with self._profile_phase_method("run_phase", phase.name):
    ...
    def _run_recipe(
        ...
        *,
        ...
        with self._profile_phase_method("stage1_video_upsample"):
            stage2_video = self._spatial_upsample_phase(stage1.video)
        with self._profile_phase_method("stage1_slots_upsample"):
        ...
        carry_keyframes = self._decode_lanczos_reencode_keyframes(stage2.generated_keyframes, request_inputs.generator)
        with self._profile_phase_method("stage2_video_upsample"):
            stage3_video = self._spatial_upsample_phase(stage2.video)
        ...
        # Match the Diffusers DFR path: slots guide Stage 3 but are not passed
        # into a joint/keyframe-aware DiffVAE decoder.
        ...
        if hasattr(output, "output") and isinstance(output.output, tuple):
