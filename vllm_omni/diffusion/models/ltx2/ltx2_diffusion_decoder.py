...
# Modified from Diffusers d035dcd7cc7c88e0a154609b62887d50bba9fdc2:
# locally owned decoder, native checkpoint loading, short-clip context and
# direct Omni operator integration (no runtime Diffusers class replacement).
...
from .ops.diffvae.qk_rms_norm import try_qk_rms_norm_scale_rope_3d_exact
from .ops.diffvae.residual_adaln import try_residual_add3_exact, try_residual_rms_norm_modulate_exact
from .ops.diffvae.swiglu import try_swiglu_tiled_exact
...
class _LTX2VideoVaeAttentionRequiredProcessor:
    def __call__(self, attn, hidden_states, block_mask=None):
        raise RuntimeError(
            "LTX-2.5 DiffVAE requires an explicitly configured CUDA attention processor. "
            "Use LTX2VideoVaeNeighborhoodNattenProcessor; no CPU/FlexAttention fallback is provided."
        )


@cache
def _load_tilelang_fna():
    from .ops.diffvae.fna import fna3d_tilelang

    return fna3d_tilelang
...
class LTX2VideoVaeNeighborhoodNattenProcessor:
    ...
    unset. Stages 1–4 use NATTEN; the 11-cubed stage uses Omni TileLang FNA.
    Unsupported native inputs or compilation failures raise; there is no CPU fallback.
    ...
    def __init__(self, backend: str | None = None):
        if DIFFUSERS_DISABLE_REMOTE_CODE:
            raise ValueError(
                ...
                "is disabled globally by the `DIFFUSERS_DISABLE_REMOTE_CODE` environment variable. "
                "Unset it to initialize this processor."
            )
    ...
    def __call__(
        ...
        if tuple(attn.kernel_size) == (11, 11, 11):
...
class LTX2VideoVaeNeighborhoodAttention(nn.Module, AttentionModuleMixin):
    rope_class = LTX2VideoVaeRotaryPosEmbed3D
    _default_processor_cls = _LTX2VideoVaeAttentionRequiredProcessor
    _available_processors = [_LTX2VideoVaeAttentionRequiredProcessor, LTX2VideoVaeNeighborhoodNattenProcessor]
    ...
    def __init__(
        ...
        self.rope = self.rope_class(head_dim, base=rope_base)
    ...
    def build_block_mask(self, hidden_states: torch.Tensor):
        # CUDA kernels encode the neighborhood directly; never build a dense
        # or FlexAttention mask for production-resolution videos.
        return None
...
class LTX2VideoVaeSwiGLU(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        optimized = try_swiglu_tiled_exact(hidden_states, self.w_gate.weight, self.w_up.weight, self.w_down.weight)
        if optimized is not None:
            return optimized
...
class LTX2VideoVaeNABlock(nn.Module):
    ...
    attention_class = LTX2VideoVaeNeighborhoodAttention
    ...
    def __init__(
        ...
        self.attn = self.attention_class(dim, kernel_size, head_dim=head_dim)
...
class LTX2VideoVaeDiffusionNABlock(nn.Module):
    ...
    def forward(
        ...
        if hidden_states.dtype is not torch.bfloat16 or not is_ltx2_ops_eligible(hidden_states):
            return self._forward_unfused(hidden_states, latent_context, modulation, block_mask)
...
class LTX2VideoDiffusionDecoder3d(nn.Module):
    ...
    deterministic_block_class = LTX2VideoVaeNABlock
    diffusion_block_class = LTX2VideoVaeDiffusionNABlock
    ...
    def __init__(
        ...
        for stage_idx, stride in enumerate(upsample_strides):
            ...
            self.det_stages.append(
                nn.ModuleList(
                    [
                        self.deterministic_block_class(
                            dim=channels,
...
class LTX2VideoDiffusionDecoderModel(ModelMixin, AttentionMixin, ConfigMixin):
    ...
    decoder_class = LTX2VideoDiffusionDecoder3d
    ...
    def decode(
        ...
        # Short clips keep replicated context through stage 5, then crop it.
        decoded = decoded[
            :,
            :,
            : (z.shape[2] - 1) * self.temporal_compression_ratio + 1,
            : z.shape[3] * self.spatial_compression_ratio,
            : z.shape[4] * self.spatial_compression_ratio,
        ]
