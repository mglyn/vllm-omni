...
This example supports startup-time prefusion and request-time dynamic LoRA.
The LoRA path must be readable on the **server** machine (usually a local path
or a mounted directory).
...
vllm serve Qwen/Qwen-Image-2512 \
  --omni \
  --port 8091 \
  --prefused-lora /path/to/Qwen-Image-2512-Lightning-4steps.safetensors
```
